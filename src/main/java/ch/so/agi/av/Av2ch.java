...
public class Av2ch {
        ...
        try {
            ...
            while (event != null) {
                ...
                if (event instanceof StartBasketEvent) {
                    ...
                    topicNr++;
                    StartBasketEvent outputStartBasketEvent = new ch.interlis.iox_j.StartBasketEvent(outputClassName,
                            Integer.toString(topicNr));

                    inputTables = ch.interlis.iom_j.itf.ModelUtilities.getItfTables(iliTdInput,
                            basket.getType().split("\\.")[0], basket.getType().split("\\.")[1]);
                    outputTables = ch.interlis.iom_j.itf.ModelUtilities.getItfTables(iliTdOutput,
                            outputStartBasketEvent.getType().split("\\.")[0],
                            outputStartBasketEvent.getType().split("\\.")[1]);
    ...
    /*
     * Checks if a table from the input model is also in the federal model.
     */
    private boolean isOutputTable(String tableName) {
        for (int i = 0; i < outputTables.size(); i++) {
            ...
            if (o instanceof Table) {
                ...
                String outputTableName = scopedOutputTableName.substring(scopedOutputTableName.indexOf(".") + 1) + "_"
                        + attrName;
                if (tableName.equals(outputTableName)) {
                    return true;
                }
                    ...
                    if (type instanceof EnumerationType) {
                        ...
                        if (attrValue != null) {
                            try {
                                ...
                                // Enum not found. We assume that this attribute does not exist in federal
                                // table.
                                // Needed for GL.
                            }
                        }
     ...
     */
    private HashMap<String, HashMap<Integer, Integer>> mapEnumerationTypes(HashMap inputEnumerations,
            HashMap outputEnumerations) {
        ...
        for (Iterator it = keys.iterator(); it.hasNext();) {
            ...
            if (cantonalEnumType == null) {
                ...
                // It will check if the first part of the federal enum name and the last part can
            ...
            ch.interlis.iom_j.itf.ModelUtilities.buildEnumList(cantonalElementList, "",
                    cantonalEnumType.getConsolidatedEnumeration());

            ArrayList federalElementList = new ArrayList();
            ch.interlis.iom_j.itf.ModelUtilities.buildEnumList(federalElementList, "",
                    federalEnumType.getConsolidatedEnumeration());
                            ...
                            if (obj instanceof Domain) {
                                ...
                                while (attri.hasNext()) {
                                    ...
                                    if (attrObj.obj instanceof AttributeDef) {
                                        ...
                                        String attrName = attrdefObj.getContainer().getScopedName(null) + "."
                                                + attrdefObj.getName();
    ...
    // credits:
    // http://www.eisenhutinformatik.ch/iox-ili/IOX-ILI-Tutorial-20091203-de.pdf
        ...
        try {
            ...
            if (ext.equalsIgnoreCase("itf")) {
                ...
                throw new IllegalArgumentException(
                        "not a valid file extension: " + new File(transferFileName).getName());
            }
